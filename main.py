...
class Tables:
    def __init__(self, jsonFileName):
        ...
        self.service = apiclient.discovery.build('sheets', 'v4', http = self.httpAuth)  # Сервис-объект для работы с Google-таблицами
        self.driveService = apiclient.discovery.build('drive', 'v3', http = self.httpAuth)  # Предоставление доступа на чтение к google таблице
        ...
        with open(LOG_FILE) as f:
            for line in f:
                ...
                results = self.service.spreadsheets().values().append(
                    spreadsheetId = self.tableID, range = "Лист1!A1:A1", valueInputOption = "USER_ENTERED", body = {"values": [[data]]}).execute()

    # Получение URL созданной таблицы
    def get_table_URL(self):
        pass

new_table = Tables(CREDENTIALS_FILE)
